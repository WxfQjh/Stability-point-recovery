...
# from models import pcn_transform_premodel
from metrics.metric import l1_cd
from metrics.loss import cd_loss_L1, emd_loss
from visualization import plot_pcd_one_view
from utils import farthest_point_sample, index_points,save_checkpoint
import numpy as np
...
def train(params):
    ...
    model = premodel.PreModel().to(params.device)
    ...
    for epoch in range(1, params.epochs + 1):
        ...
        for i, (p, c) in enumerate(train_dataloader):
            ...
            p = c.to(params.device)
            choice = np.random.choice(16384, 2048, replace=False)
            p = p[:, choice, :]
            ...
            if params.coarse_loss == 'cd':
                ...
                choice = np.random.choice(16384, 2048, replace=True)
                coarse_c = c[:, choice, :]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('PCWGAN_Transformer')
    ...
    parser.add_argument('--coarse_loss', type=str, default='emd', help='loss function for coarse point cloud')
