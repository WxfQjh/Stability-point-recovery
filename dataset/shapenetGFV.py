...
class ShapeNetGFV(data.Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        partial_GFV = np.load(partial_path)
        complete_pc = np.load(complete_path)

        return torch.from_numpy(partial_GFV), torch.from_numpy(complete_pc)
    ...
    def _load_data(self):
        ...
        for line in lines:
            ...
            if self.split == 'train':
                partial_paths.append(os.path.join(self.dataroot, 'partial_gfv', category, model_id + '_{}.npy'))
            else:
                partial_paths.append(os.path.join(self.dataroot, 'partial_gfv', category, model_id + '.npy'))
            complete_paths.append(os.path.join(self.dataroot, 'complete_gfv', category, model_id + '.npy'))
